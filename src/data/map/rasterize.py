...
from utils.config import METER_CRS, DEFAULT_CRS

class Rasterizer(nn.Module):
    """Discretizes spatial and kinematic vessel features into grid indices."""

    def __init__(self, bbox, pos_res = 50):
        super().__init__()
        transformer = pyproj.Transformer.from_crs(
            pyproj.CRS(DEFAULT_CRS), pyproj.CRS(METER_CRS), always_xy=True
        )
    ...
    def get_total_grid_sizes(self):
        """Return grid sizes for position and dynamic feature dimensions."""

        return self.x_size, self.y_size, self.sog_size, self.cog_size, self.acc_size, self.rot_size

    def pos_to_index(self, pos):
        """Convert metric positions to clamped spatial grid indices."""
    ...
    def pos_to_grid_coords(self, pos):
        """Map metric coordinates to continuous grid coordinates."""
    ...
    def kin_to_index(self, feat):
        """Discretize speed over ground (SOG) and course over ground (COG)."""
    ...
    def dyn_to_index(self, feat):
        """Discretize acceleration and rotation using predefined quantile bins."""
