...
def shape_aware_cpa_and_min_dist(
    ...
    return risk, dcpa_hull, min_hull_dist
...
def compute_batch_collision_risk(data, pred_abs):
    ...
    # ---------------------------------------------------------
    risk, dcpa_hull, min_hull_dist = shape_aware_cpa_and_min_dist(
        ego_tensor,
        other_tensor,
        ego_static,
        other_static,
    ...
    # ---------------------------------------------------------
    # Dynamic close-distance threshold (mean beam)
    # ---------------------------------------------------------
    ego_static_denorm = denormalize_static(ego_static)
    other_static_denorm = denormalize_static(other_static)

    ego_beam = ego_static_denorm[:, 2] + ego_static_denorm[:, 3]
    other_beam = other_static_denorm[:, 2] + other_static_denorm[:, 3]

    pair_close_threshold = 0.5 * (ego_beam + other_beam)
    ...
    graph_close_threshold = torch.zeros(num_graphs, device=device)
    graph_close_threshold = graph_close_threshold.scatter_reduce(
        0, graph_ids, pair_close_threshold, reduce="amax"
    )
    ...
    graph_close_threshold = graph_close_threshold[valid_graphs]

    high_risk_count = (graph_max_risk > 0.90)
    close_dist_count = (graph_min_dist < graph_close_threshold)
