...
class SocialPool(nn.Module):
    ...
    def forward(self, pred_pos_abs, hidden, data):
        ...
        for ego in ego_idx:
            ...
            # ---- positions at t0 ----
            ego_pos = pred_pos_abs[ego, :, 0]       # [K,2]
            nbr_pos = pred_pos_abs[nbrs, :, 0]      # [M,K,2]
            nbr_hid = hidden[nbrs, :, 0]            # [M,K,H]

            # relative geometry per hypothesis
            diff = nbr_pos - ego_pos.unsqueeze(0)   # [M,K,2]
            r = torch.norm(diff, dim=-1)            # [M,K]
