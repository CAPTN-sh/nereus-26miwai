...
class CVAEEncoderParams:
    ...
    def __post_init__(self):
        self.layer_sizes = [(self.cond_size + self.latent_size, 48), (48, 48), (48, 48)]
...
class CVAEDecoderParams:
    ...
    def __post_init__(self):
        self.layer_sizes = [(self.cond_size + self.latent_size, 48), (48, 48)]
...
class CVAEParams:
    ...
    def __post_init__(self):
        self.encoder_params = CVAEEncoderParams(
            cond_size=self.cond_size, latent_size=self.latent_size
        )
        self.decoder_params = CVAEDecoderParams(
            cond_size=self.cond_size, latent_size=self.latent_size
        )
...
class SocialPoolingParams:
    ...
    def __post_init__(self):
        self.fc_config = [
            self.num_rings * self.num_wedges * self.hidden_size,
            self.hidden_size,
            nn.ReLU(),
        ]
...
class PoolingCNNParams:
    layer_configuration: List = field(
        default_factory=lambda: [
            (3, 16, (5, 5), 2, nn.Conv2d, nn.ReLU()),
...
@dataclass
class SCFParams:
    scene_size: tuple = field(default_factory=lambda: (512, 512, 2))
    input_size: int = 2
    intermediate_size: int = 16
    sp_params: SocialPoolingParams = field(
        default_factory=lambda: SocialPoolingParams()
    )

    def __post_init__(self):
        self.velocity_fc = [(self.input_size, self.intermediate_size, nn.ReLU())]
...
class SGMParams:
    ...
    cvae_params: CVAEParams = field(default_factory=CVAEParams)
