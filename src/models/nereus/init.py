...
from models.nereus.prior_modules.prior import MAP_GMM, DensityMap
...
from utils.config import AIS_SOURCE, DATA_FOLDER_PATH, TRAIN_BBOX
...
def load_traisformer_prior(device):
    ...
    prior_module = TrAISformerHeatmap(ckpt["config"])
...
def load_gmm_prior(rasterizer, device, n_cluster = 16):
    ...
    ckpt = torch.load(best_ckpt_path, map_location=device,weights_only=False)
    trais = TrAISformerHeatmap(ckpt["prior_config"]).to(device)
...
def build_nereus(
    ...
    map_mod: str | None = None,
    prior: str | None = None,
    rasterizer=None,
):
    """Build NEREUS from explicit module choices (social/map/prior as strings or None)."""
    if not rasterizer:
    ...
    return NEREUS(config=cfg, static_module=True, social_module=social_module, map_module=map_module, prior_module=prior_module)
